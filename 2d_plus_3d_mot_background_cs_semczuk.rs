...
// 2mm: 284.55        0.75524        -25.754         17.767 2.5mm: 299	      0.69      	 -25	         18.87 3mm: 241.64        0.28328        -22.994         19.692 4mm: 298.43        0.799          -25.239         17.665 5mm: 299.22        0.57828        -25.386         13.191 6mm 285.11         0.70926        -26.056         8.552 7.5mm: 285         0.65289        -25.218         5.0792
    // Magnetic gradient
    let quadrupole_gradient_2d = 8.386;
    let quadrupole_gradient_3d = 8.5792;

    // Push beam along x axis
    let detuning_push= -25.386; //-26.232
    let power_push = 0.885678766695696;
    let radius_push = 1.40285259522301; // 1.5mm 1/e^2 radius because the hole limits it
    let wavelenght= 852.0e-9;

    // // Create cooling lasers - 2DMOT
    let detuning_2d = detuning_push;
    let power_2d = 299.22;
    // let radius_2d = 4.78294334304728; It makes the 2D trap the most efficient but does it create the highest atomic flux
    let radius_2d = 5.5;
    let ellipticity = 0.842044819918941;

    let detuning_3d = detuning_push;
    // let power_3d = 95.0;
    let radius_3d = 7.5;

    let ratio_power_3d_2d = 0.65289;
