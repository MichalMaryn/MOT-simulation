title = '2d_plus_3d_mot_background_cs_semczuk';
...
z_center = 0.00149608569809914;  % z-coordinate of the centre

a = 0.00478294334304728;      % Semi-axis length along x
b = 0.002584855354503405;      % Semi-axis length along y
c = 0.002584855354503405;      % Semi-axis length along z
...
gradient_2D = 8.386; % Example gradient
detuning_2D = 25.386; % Detuning in MHz
gradient_3D = 8.5792; % Example gradient
detuning_3D = 25.386; % Detuning in MHz
...
trapping_rate = a*double(length(ids_2))/double(10000000.0)*((S*(p))/(2.0*3.14*m*k*T)^(0.5));
display(trapping_rate);
display(a*double(length(ids_pos_5))/double(10000000.0)*((S*(p))/(2.0*3.14*m*k*T)^(0.5)));
